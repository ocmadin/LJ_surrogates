...
def main(n_workers, cpus_per_worker, gpus_per_worker):
    results = run_server(n_workers, cpus_per_worker, gpus_per_worker)
    os.makedirs('estimated_results', exist_ok=True)
    for i, result in enumerate(results):
        result.estimated_properties.json(
            (os.path.join('estimated_results', "estimated_data_set_" + i + '.json')), format = True)
...
main(args.workers, args.cpus_per_worker, args.gpus_per_worker, args.dir)
